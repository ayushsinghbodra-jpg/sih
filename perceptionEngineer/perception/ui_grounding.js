...
class UIGroundingEngine {
  constructor(options = {}) {
    this.modelPath = options.modelPath || './models/yolo11n.onnx';
  ...
  async extractElementsAsync(imageSource, domTree) {
    if (this.backend !== 'dom-fallback' && this.onnxSession) {
      try {
        ...
        if (visionBoxes && visionBoxes.length > 0) {
          ...
          return this._mergeVisionWithDOM(visionBoxes, domTree);
        }
      } catch (e) {
    ...
    return this.extractElements(imageSource, domTree);
  }
  ...
  extractElements(imageSource, domTree) {
    ...
    if (typeof domTree === 'string') {
      const parsedDOM = this._parseHTMLString(domTree);
      if (parsedDOM) {
        this._traverseDOMTree(parsedDOM, elements);
      }
    } else if (domTree && typeof domTree === 'object' && !domTree.querySelectorAll) {
      this._traverseDOMTree(domTree, elements);
    } else if (domTree && domTree.querySelectorAll) {
      this._extractFromLiveElement(domTree, elements);
